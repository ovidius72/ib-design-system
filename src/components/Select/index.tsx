...
const IBSelect: React.FunctionComponent = () => {
  const [selectableItemType, setSelectableItemType] = React.useState<
    'all' | 'vowels'
  >('all');
  ...
  const items = useConst<any[]>(() => createListItems(ITEM_COUNT));
    ...
    () => [
      {
        ...
        subMenuProps: {
          items: Object.keys(selectionModes).map(
            (mode: keyof typeof SelectionMode) => ({
              key: mode,
      ...
      },
      {
        ...
        subMenuProps: {
          items: Object.keys(selectableItemTypes).map(
            (itemType: keyof typeof selectableItemTypes) => ({
              key: itemType,
